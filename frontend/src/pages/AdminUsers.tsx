...
function InlineSelect<T extends string>({
  value, options, labels, onChange, disabled,
}: {
  ...
  return (
    <select
      ...
      style={{ backgroundColor: "rgba(255,255,255,0.06)", color: "var(--text-primary)", appearance: "auto" }}
    >
      {options.map((opt) => (
...
function UserRow({
  user, isSelf, onUpdate, onRefreshQuota,
}: {
  user: AdminUser;
  isSelf: boolean;
    ...
    <tr style={{ borderBottom: "1px solid var(--border)" }}>
      <td className="py-3 pr-4 pl-4">
        <div className="flex flex-col gap-0.5">
          <span className="text-[13px] font-medium" style={{ color: "var(--text-primary)" }}>
            {user.email}
            {isSelf && (
              <span className="ml-2 text-[10px] font-normal" style={{ color: "var(--text-tertiary)" }}>(you)</span>
            )}
          </span>
        ...
        {quota ? (
          <div className="flex flex-col gap-1">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px]" style={{ color: "var(--text-tertiary)" }}>{usedPct}%</span>
            </div>
      ...
      <td className="py-3 pr-4">
        <button
          ...
          onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.backgroundColor = "rgba(255,255,255,0.10)"; (e.currentTarget as HTMLElement).style.color = "var(--text-primary)"; }}
          onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.backgroundColor = "rgba(255,255,255,0.06)"; (e.currentTarget as HTMLElement).style.color = "var(--text-secondary)"; }}
        >
          Reset quota
        </button>
      </td>
...
export default function AdminUsers() {
  ...
  // Use cached me (already fetched by AdminLayout)
  const { data: me } = useQuery<UserMeResponse>({
    queryKey: ["me"],
    queryFn: async () => {
      const token = await getToken();
      return apiFetch("/users/me", { token: token! }).then((r) => r.json());
    },
    staleTime: 60_000,
  });
  ...
  const { mutate: updateUser } = useMutation({
    mutationFn: async ({ id, patch }: { id: string; patch: object }) => {
      const token = await getToken();
      const res = await apiFetch(`/admin/users/${id}`, { method: "PATCH", token: token!, body: JSON.stringify(patch) });
      if (!res.ok) {
  ...
  const { mutate: refreshQuota } = useMutation({
    mutationFn: async (id: string) => {
      const token = await getToken();
      const res = await apiFetch(`/admin/users/${id}/quota/refresh`, { method: "POST", token: token! });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <h1 className="text-[22px] font-bold tracking-tight" style={{ color: "var(--text-primary)" }}>Users</h1>
          <p className="text-[13px] mt-1" style={{ color: "var(--text-secondary)" }}>
            Manage roles, access levels, and quotas.
          </p>
        </div>
        <input
          type="search"
          ...
          className="text-[13px] rounded-xl px-3 py-2 outline-none w-full sm:w-60"
      ...
      {!isLoading && users.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {([
            ...
            { label: "Pro+Ent",      value: (levelCounts.pro ?? 0) + (levelCounts.enterprise ?? 0) },
          ] as { label: string; value: number }[]).map(({ label, value }) => (
            <div key={label} className="rounded-2xl px-4 py-3" style={{ backgroundColor: "var(--surface)", border: "1px solid var(--border)" }}>
              <p className="text-[10px] uppercase tracking-widest font-semibold mb-1" style={{ color: "var(--text-tertiary)" }}>{label}</p>
              <p className="text-[22px] font-bold tabular-nums" style={{ color: "var(--text-primary)" }}>{value}</p>
            </div>
          ))}
        </div>
      )}
              ...
              <thead>
                <tr style={{ borderBottom: "1px solid var(--border)" }}>
                  {["User", "Role", "Access Level", "Enabled", "Jobs", "Quota (this month)", ""].map((h) => (
                    <th key={h} className="px-0 pr-4 py-3 text-left text-[10px] uppercase tracking-widest font-semibold first:pl-4" style={{ color: "var(--text-tertiary)" }}>
                      {h}
                    </th>
                  ))}
                </tr>
